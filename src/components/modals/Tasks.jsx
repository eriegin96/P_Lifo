...
export default function Tasks() {
    ...
    if (isBreak) {
      ...
      updateUser(uid, {
        currentSession: {
          ...
          pomodoroTime:
            currentSession.pomodoroTime + initPomodoroTime * 60 - pomodoroTime,
        },
      });
    }
  };
  ...
  const unsetCurrentTask = () => {
    const newTaskList = currentSession.taskList.map((task) => ({
      ...task,
      current: false,
    }));
              ...
              <div className='w-1/2'>
                ...
                <div className='my-4 w-[140px] h-[50px] rounded-lg bg-bg-200 flex items-center justify-center'>
                  <img
                    ...
                    onClick={() => {
                      updateUser(uid, {
                        timer: { ...timer, pomodoroTime: initPomodoroTime - 1 },
                      });
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateUser(uid, {
                        timer: { ...timer, pomodoroTime: e.target.value },
                      })
                    }
              ...
              <div className='w-1/2'>
                ...
                <div className='my-4 w-[140px] h-[50px] rounded-lg bg-bg-200 flex items-center justify-center'>
                  <img
                    ...
                    onClick={() => {
                      updateUser(uid, {
                        timer: { ...timer, breakTime: initBreakTime - 1 },
                      });
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateUser(uid, {
                        timer: { ...timer, breakTime: e.target.value },
                      })
                    }
                    className='w-1/3 h-full bg-bg-200 text-center'
                  />
                        ...
                        {ALARM_LINKS.map((alarm, idx) => (
                          <Listbox.Option
                            ...
                            className={({ active }) =>
                              `${
                                active
                                  ? ' bg-[rgba(243,169,82,0.33)] rounded-md '
                                  : ''
                              }
                      ...
                      {((initBreakTime * 60 === breakTime && !isBreak) ||
                        ...
                        <Button onClick={skipTimer} className='mx-4'>
                          <img
                            src={skipIcon}
                            alt='skip'
                            className='w-9 h-[30px] invert'
                          />
                        </Button>
                      )}

                      {initBreakTime * 60 > breakTime && !isBreak && (
                        <p>You are in Break time</p>
                      )}
                          ...
                          <Menu.Button className='align-middle'>
                            {({ open }) => (
                              <div
                                className={`${
                                  open
                                    ? 'bg-transparent-w-20 rounded-md opacity-100'
                                    : 'opacity-50'
