...
export default function AppProvider({ children }) {
  ...
  const [draggableModalType, setDraggableModalType] = useState(
  ...
  useEffect(() => {
    if (isPomodoroTimePlaying) {
      if (pomodoroTime === 0) {
        ...
        const timeout = setTimeout(() => alarmRef.current.pause(), 10000);
      ...
      if (breakTime === 0) {
        ...
        updateUser(user?.uid, {
          currentSession: {
            ...
            breakTime:
              currentSession.breakTime + initBreakTime * 60 - breakTime,
          },
        });
        return () => {
